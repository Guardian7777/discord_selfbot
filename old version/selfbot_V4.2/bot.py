...
async def 관리(ctx):
    ...
    message = (
        ...
        f"> **4️⃣ gpt: gpt에게 질문하려면 {prefix}gpt 내용 을 입력하세요**\n"
        f"> **5️⃣ 구글: 구글 검색을 하려면 {prefix}구글 내용 을 입력하세요**\n"
    )
    await ctx.reply(message)
...
# OpenAI API 키 설정
openai.api_key = '' # 콘픽 설정에서 하거나 여기서 해도 됨
...
"## 브롤스타즈 API : 내 계정을 확인하려면 명령어 앞에 내 를 입력하세요\n"
        ...
        f"> **7️⃣ 추천: 브롤러 추천을 받으시려면 {prefix}추천천 를 입력하세요**\n"
    )
    await ctx.reply(message)
